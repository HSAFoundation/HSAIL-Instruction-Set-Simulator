...
#include <iostream>
    ...
    #else
        #define DLL_PUBLIC __declspec(dllexport) // Note: actually gcc seems 
...
typedef enum
{
    ...
    /**
     * @brief Device composed of latency compute units.  Latency devices
     *        support both the native host instruction set as well as the
     *        HSAIL instruction set.  CPUs are considered latency devices.
     */
    DEVICE_TYPE_LATENCY = (1 << 1),

    /**
     * @brief Device composed of throughput compute units.  Throughput devices
     *        only support the HSAIL instructions set and typically have 
     *        dedicated hardware for very efficient parallel operations.  GPUs
     *        and specialized processors are usually considered throughput 
     *        devices.
     */
